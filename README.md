...
> Center Loss, SupCon, MC Dropout, ensemble 등은 val/test에서 유의미한 개선이 없어 최종 노트북에 포함하지 않았습니다.
...
- Argmax는 unknown recall 0 → open-set에서 거부 불가
...
```python
RETRAIN = False       # True: scratch 학습 (결과가 달라질 수 있음)
...
```bash
jupyter nbconvert --to pdf notebook.ipynb --output notebook_output.pdf
```
...
├── notebook_output.pdf     # 노트북 PDF export
├── model.pth               # checkpoint + Mahalanobis stats (실행 후 생성)
├── report.pdf              # 보고서 (optional)
├── requirements.txt
├── README.md
└── outputs/                # 실행 결과
    ├── final_ranking.csv
...
`Data/`, `model.pth`, `outputs/best_model.pth`는 `.gitignore` 대상 (용량·데이터 정책).
